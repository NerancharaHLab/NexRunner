...
# Base clean scenarios template without awkward site suffixes in desc, steps, criteria
clean_scenarios = [
    {
        "id": "SC-01",
        "flow": "OPD",
...
# Write clean JSON scenario files for all sites (NUH, SBH, TMH, Siriraj, Standard)
site_names = {
    "nuh.json": ("NUH", "NUH (โรงพยาบาลมหาวิทยาลัยนเรศวร)"),
    "sbh.json": ("SBH", "SBH (โรงพยาบาลสระบุรี)"),
    "tmh.json": ("TMH", "TMH (โรงพยาบาลเวชศาสตร์เขตร้อน)"),
    "siriraj.json": ("Siriraj", "Siriraj Hospital (รพ.ศิริราช)"),
    "standard.json": ("Standard", "Standard (General Hospital)")
}

scenarios_embedded = {}

for fname, (code, full_name) in site_names.items():
    sc_list = []
    for sc in clean_scenarios:
        item = dict(sc)
        item['name'] = f"{code}: {sc['name']}"
        sc_list.append(item)
    ...
    scenarios_embedded[code] = data
    
    with open(f'./scenarios/{fname}', 'w', encoding='utf-8') as f:
        json.dump(data, f, ensure_ascii=False, indent=2)

print('Cleaned all JSON scenario files in ./scenarios/')

# Update js/app.js embedded scenarios bundle and loadFallbackScenarioData
...
# Replace EMBEDDED_SCENARIOS block in app.js
start_mark = "// Embedded Scenarios & Sites Registry Fallback"
end_mark = "let appConfig ="

start_idx = app_js.find(start_mark)
end_idx = app_js.find(end_mark)
...
print('Successfully cleaned site suffixes from all scenario texts in JSON and js/app.js.')
