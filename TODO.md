...
- [ ] **[P4]** [REQ-022: CSV/Excel Scenario Import](specs/REQ-022_csv_excel_scenario_import.md) — 🚧 Phase 1 (backend core: `papaparse` parse + validate + transaction-safe commit, `lib/scenario-import.ts`) done and verified via script (build+e2e 27/27, all-or-nothing + zero-sequence-waste-on-reject confirmed against real Postgres). Corrected 3 points in the user's BA+SA brief before implementing (transaction composability didn't exist yet in `id-sequence.ts`/`createScenario()` — added as optional params; "no ID gaps" isn't an actual REQ-032 requirement; no CSV/Excel lib existed — scoped to CSV only). Still needed: Upload UI + Template download + Preview modal + Excel support — no route/UI wired up yet
