...
- [x] **[P2]** (bookkeeping-only, no new REQ — closes verification debt on REQ-039/REQ-040) Permanent Playwright E2E coverage for Suite Management (site scoping) + Scenario Board Steps/Criteria
  - [x] `e2e/global-setup.ts`: เพิ่ม `E2E_SITE_KEY_2` ("E2EB") site ที่สอง (ถาวร) สำหรับเทส cross-site scoping
  - [x] ใหม่ `e2e/tests/10-suite-management.spec.ts` (4 tests: create+picker scope badges, list search/filter, delete-guard plain-vs-warning against a real in-use Suite, cross-site scoping)
  - [x] ใหม่ `e2e/tests/11-scenario-board-steps-criteria.spec.ts` (2 tests: per-card independent expand/collapse w/ real content assertions, works on locked Run)
  - [x] อัปเดต spec REQ-039/REQ-040 ชี้ไปที่ e2e spec ใหม่
  - [x] Verify: build clean + `npm run test:e2e` 33/33 รันซ้ำ 2 รอบ (idempotent ยืนยันแล้ว) + Postgres confirm ไม่มี scratch data ค้าง + manual Playwright walkthrough หน้าที่เหลือ (Manage Tags/Environments/Sites/Users, Master Library, Clone-from-Master, CSV Import modal, Executive Report print) ผ่านหมด 0 console error — เจอ+แก้ script ตัวเองพลาด 1 จุด (generic `button[type="submit"]` selector ดันไปกด TopNav logout ก่อนปุ่ม save จริง เพราะ DOM order มาก่อน ไม่ใช่ product bug) ก่อนสรุปว่าผ่าน
- [x] **[P3]** (prep for REQ-027, no new REQ) DevOps handoff doc + production Dockerfile — user ยืนยัน Postgres จะได้ schema ใหม่แยกใน `dev_cortex` (DB จริงของ Cortex/HIS) เดียวกัน ไม่ใช่ DB แยก (ดูภาพหน้าจอ `dev_cortex_readonly` ที่ user ส่งมา — มี schema `ehr`/`hivent`/`user_management`/ฯลฯ อยู่แล้ว)
  - [x] อัปเดต `specs/REQ-027_provision_azure_deploy.md` บันทึกการตัดสินใจ DB ใหม่ ชี้ไปที่ handoff doc
  - [x] ใหม่ `docs/devops_handoff.md` (schema placement, env var 3 ตัว, permission ที่ต้องเป็น write ไม่ใช่ readonly, migrate/seed command, verification checklist)
  - [x] `web/next.config.ts`: เพิ่ม `output: "standalone"`
  - [x] ใหม่ `web/Dockerfile` (multi-stage: `deps`→`builder`→`runner` + แยก `migrator` target สำหรับรัน `prisma migrate deploy`) + `web/.dockerignore`
  - [x] Verify: `npm run build` (standalone) ผ่าน, e2e 33/33 ไม่พัง, `docker build` ทั้ง 2 target ผ่านจริง, และรัน `migrator` image จริงกับ Docker Postgres local (schema ใหม่ `?schema=` ผ่าน migrate 6 ตัวสำเร็จ) + boot `runner` image จริงได้ HTTP 200 บน `/login` — เจอ+แก้ 2 จุดจริงระหว่างทาง: (1) `prisma generate` ต้องมี `DATABASE_URL` แม้จะไม่ได้ต่อ DB จริง (ต้องเติม build-time ARG placeholder) (2) ลอง copy `node_modules/prisma` แบบเลือกเฉพาะเข้า runner image เพื่อรัน migrate จาก image เดียวกัน แล้วพบว่า MODULE_NOT_FOUND จริง จึงแยกเป็น `migrator` stage ต่างหากแทน — cleanup scratch schema/image/user หมดแล้ว
- [x] **[P2]** [REQ-041: Migrate Evidence storage from Azure Blob Storage to SeaweedFS](specs/REQ-041_seaweedfs_evidence_storage.md) — user ตัดสินใจจริง ต่อผ่าน S3-compatible API (`@aws-sdk/client-s3`) — ใหม่ `lib/storage/blob.ts` แทน `lib/azure/blob.ts`, env var เปลี่ยนเป็น `S3_ENDPOINT`/`S3_ACCESS_KEY_ID`/`S3_SECRET_ACCESS_KEY`/`S3_BUCKET`, local dev รัน SeaweedFS จริงผ่าน `docker-compose.yml` (ลบ `azurite` devDependency ทิ้งทั้งหมด, ลบไฟล์ `.azurite/` 211MB ทิ้ง), อัปเดตเอกสารครบ (README/devops_handoff/REQ-027) + redeploy System Map artifact — build clean + e2e 33/33 (รอบแรกมี 5 fail ไม่เกี่ยวกับ storage เลย เช็คแล้วเป็น resource contention จาก manual test ก่อนหน้า ไม่ใช่ regression จริง รันซ้ำผ่านหมด) + manual script ยืนยัน object จริงถูกสร้าง/ลบใน SeaweedFS ผ่าน UI จริง (ไม่ใช่แค่ UI ดูเหมือนถูก) — เจอ+แก้บั๊กจริงระหว่างทาง: healthcheck ใช้ `localhost` แต่ resolve เป็น IPv6 ก่อนใน container ทำให้ connection refused ตลอด แก้เป็น `127.0.0.1`
...
- [x] **[P4]** (bookkeeping-only, no spec file) `web/Dockerfile` + `web/.dockerignore` still said
  ...
  SeaweedFS — fixed both comments to the real `S3_*` env vars. Rebuilt both the `runner` and
  `migrator` targets fresh against the post-REQ-041 dependency tree (azurite removed,
  `@aws-sdk/client-s3` added). First attempt hit a real environment failure mid-verification: the
  disk was at 100% capacity (118MB free of 228GB), which had also corrupted Docker Desktop's own
  storage and then took its daemon down entirely — did not keep retrying against a full disk,
  flagged it to the user instead. After the user freed disk space (118MB → 27GB): restarted Docker
  Desktop clean (had to force-kill stale zombie processes left over from the disk-full crash before
  a fresh launch would actually take), then **re-verified both targets for real**: `migrator` run
  against a real local Postgres scratch schema applied all 6 real migrations successfully; `runner`
  booted against that migrated schema + the real SeaweedFS container with the new `S3_*` vars,
  real `200` on `/login`. Pruned 12.8GB of Docker build cache afterward (a real contributor to the
  disk-full crisis) and cleaned up all scratch schemas/images
