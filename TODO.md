...
- [x] **[P2]** [REQ-035: Manage Tags UX Redesign — Usage Count + Safe Delete + Search](specs/REQ-035_manage_tags_ux_redesign.md) — Usage count (Master-only scope), Hard Block delete when in-use (server + UI), search + stats bar + pill styling. Found+fixed a real bug along the way: native `disabled` buttons never fire `onClick`, which would've made the "why is this blocked" modal impossible — switched to a styled-but-clickable `.btn-danger-text--blocked`. build+e2e 24/24+manual Puppeteer verify (screenshots show the red/grey Delete button distinction clearly) all pass
