...
- [ ] **[P2]** [REQ-039: Suite Management Enhancements (Search, Site Scoping & Scenario Picker Ergonomics)](specs/REQ-039_suite_management_enhancements.md) — 📋 Spec ล็อกแล้ว ผ่านหลายรอบตรวจกับโค้ด/DB จริง (พบ+แก้ 2 จุด: `deleteSuite()` ไม่มี usage-guard เลย, และ SRS อ้างถึง Steps/Criteria expandable component บนหน้า Run ที่ไม่มีอยู่จริงในระบบ — grep แล้วไม่เจอที่ไหนเลย ต้องสร้างใหม่ทั้งหมด) เพิ่ม `Suite.siteId` (FK จริงไป Site, ไม่ใช่ free-text แบบ `sourceSite`), backfill Suite เดิม 5 ตัวตามข้อมูลจริง (NUH×4, TMH×1), soft delete-guard, Scenario Picker ใหม่ reuse REQ-038's accordion แต่สร้าง `ScenarioExpandableRow` ใหม่ทั้งหมด, และ New Run page's Suite picker กรองตาม Site จริง (จุดประสงค์เดิมของทั้ง REQ) — ยังไม่ implement รอ `EnterPlanMode`
