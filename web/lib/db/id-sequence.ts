 ...
 */
async function nextRunningNumber(scope: string): Promise<number> {
...
export async function nextMasterScenarioId(): Promise<string> {
  return `MST-${pad4(await nextRunningNumber("MST"))}`;
}

/** SUT-0001, SUT-0002, ... — global, never reset. */
export async function nextSuiteId(): Promise<string> {
  return `SUT-${pad4(await nextRunningNumber("SUT"))}`;
}
...
export async function nextCustomScenarioId(siteKey: string): Promise<string> {
  return `${siteKey}-CUST-${pad4(await nextRunningNumber(`CUST:${siteKey}`))}`;
}

/** RUN-{SITE}-0001, ... — per-site counter, never reset. */
export async function nextRunId(siteKey: string): Promise<string> {
  return `RUN-${siteKey}-${pad4(await nextRunningNumber(`RUN:${siteKey}`))}`;
}
